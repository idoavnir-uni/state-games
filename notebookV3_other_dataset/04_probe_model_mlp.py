...
COLOR_TOKEN_IDS = torch.tensor([color_token_ids[c] for c in dataset.cities])
...
for idx in tqdm(range(len(dataset)), desc="Processing samples"):
    ...
    metadata_rows.append({
        ...
        'target_color': sample.fixed_entity_city,
...
print(f"Target city: {df.iloc[0]['target_color']}")
...
COLOR_TO_IDX = {color: idx for idx, color in enumerate(dataset.cities)}
labels = torch.tensor([COLOR_TO_IDX[c] for c in df['target_color']])
n_colors = len(dataset.cities)
...
if RETRAIN_DATASET_SIZE is not None:
    ...
    for idx in tqdm(range(RETRAIN_DATASET_SIZE), desc="Extracting states"):
        ...
        retrain_colors.append(sample.fixed_entity_city)
...
for sample_idx, sample in enumerate(viz_samples):
    ...
    true_color_idx = COLOR_TO_IDX[sample.fixed_entity_city]
