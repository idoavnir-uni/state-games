...
In GLA \cite{yang2023gla}, the state update and query take the form:
\begin{equation}
...
\paragraph{Mamba-2.}
In Mamba-2 \cite{dao2024ssmduality}, the formulation is:
\begin{equation}
...
We construct \emph{in-context} entity-binding datasets \cite{feng2024binding} to study how SSMs store and retrieve factual associations. Each dataset follows a generic structure: samples consist of $N_e$ statements of the form ``\texttt{<entity> <relation> <property>}'', where each entity is associated with one of $C$ possible property values.
...
To establish that the heads identified by probing have a \emph{causal} effect on model behavior, we perform counterfactual patching experiments \cite{zhang2023patching} using the prompt structure from \cref{sec:model_retrieval}.
