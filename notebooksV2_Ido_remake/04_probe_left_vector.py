# %% [markdown]
# # Probe B: Left-Vector Factorization
#
# This notebook trains linear probes on RWKV state matrices using left-vector factorization.
#
# **Probe model:** logits = (w_left @ state) @ W_right
# - state: (batch, head_size, head_size) per head
# - w_left: (head_size,) learned vector
# - W_right: Linear(head_size, n_classes) learned matrix
...
# %% [markdown]
# ## 6. Define Left-Vector Factorization Probe
#
# logits = (w_left @ state) @ W_right


# %%
class LeftVectorProbe(nn.Module):
    def __init__(self, head_size, n_classes):
        super().__init__()
        self.w_left = nn.Parameter(torch.randn(head_size) * 0.01)
        self.W_right = nn.Linear(head_size, n_classes)

    def forward(self, state):
        # state: (batch, head_size, head_size)
        # state.T @ w_left: (batch, head_size, head_size) -> transpose -> (batch, head_size, head_size) @ (head_size,) -> (batch, head_size)
        hidden = state.transpose(-2, -1) @ self.w_left
        # W_right(hidden) -> (batch, n_classes)
        logits = self.W_right(hidden)
        return logits
...
def train_probe(states_np, labels, layer_idx, head_idx, patience=10):
    ...
    probe = LeftVectorProbe(head_size, n_classes).to(device)
...
best_probe = LeftVectorProbe(head_size, n_classes).to(device)
...
plt.tight_layout()
plt.savefig("../data/rwkv_left_vector_probe_accuracy_by_position.png", dpi=150)
plt.show()
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig("../data/rwkv_left_vector_probe_accuracy_rolling_window.png", dpi=150)
plt.show()
