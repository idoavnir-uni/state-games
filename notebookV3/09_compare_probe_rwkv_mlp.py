...
# %%
# Cross-Dataset Head Testing - RWKV MLP Probes
# 
# Goal: Test if RWKV MLP probe heads important for one task are important for that task only
...
# %%
print("\n" + "=" * 80)
print("LOADING RWKV MLP PROBE RESULTS")
print("=" * 80)
...
# Load RWKV MLP results from FavoriteColor
results_color_rwkv = pd.read_csv('results/result_rwkv_mlp.csv')
heads_color_rwkv = df_to_heads(results_color_rwkv, NUM_HEADS_TO_TEST)
print(f"\nFavoriteColor RWKV MLP heads: {heads_color_rwkv}")
print(f"  Best val_acc: {results_color_rwkv.iloc[0]['val_acc']:.3f}")

# Load RWKV MLP results from LivesInCity
results_city_rwkv = pd.read_csv('../notebookV3_other_dataset/results/result_rwkv_mlp.csv')
heads_city_rwkv = df_to_heads(results_city_rwkv, NUM_HEADS_TO_TEST)
print(f"\nLivesInCity RWKV MLP heads: {heads_city_rwkv}")
print(f"  Best val_acc: {results_city_rwkv.iloc[0]['val_acc']:.3f}")
...
# %%
print("\n" + "=" * 80)
print("CROSS-DATASET HEAD TESTING - RWKV MLP PROBES")
print("=" * 80)

all_results = []
n_total = NUM_SAMPLES_TO_TEST

# Test FavoriteColor RWKV MLP heads on both datasets
print("\nTesting FavoriteColor RWKV MLP heads...")
result = run_hook_experiment(sample_pairs_color, heads_color_rwkv, model, tokenizer, n_layer)
all_results.append({
    'heads_from': 'FavoriteColor RWKV',
    'heads': str(heads_color_rwkv),
...
result = run_hook_experiment(sample_pairs_city, heads_color_rwkv, model, tokenizer, n_layer)
all_results.append({
    'heads_from': 'FavoriteColor RWKV',
    'heads': str(heads_color_rwkv),
...
# Test LivesInCity RWKV MLP heads on both datasets
print("Testing LivesInCity RWKV MLP heads...")
result = run_hook_experiment(sample_pairs_color, heads_city_rwkv, model, tokenizer, n_layer)
all_results.append({
    'heads_from': 'LivesInCity RWKV',
    'heads': str(heads_city_rwkv),
    'tested_on': 'FavoriteColor',
    'original': result['original'],
    'switched': result['switched'],
    'neither': result['neither'],
...
result = run_hook_experiment(sample_pairs_city, heads_city_rwkv, model, tokenizer, n_layer)
all_results.append({
    'heads_from': 'LivesInCity RWKV',
    'heads': str(heads_city_rwkv),
    'tested_on': 'LivesInCity',
...
# %%
print("\n" + "=" * 100)
print("FINAL RESULTS TABLE - RWKV MLP PROBES")
print("=" * 100)
...
# %%
print("\n" + "=" * 100)
print("ANALYSIS: SAME-TASK vs CROSS-TASK (RWKV MLP)")
print("=" * 100)

print("\n### FavoriteColor RWKV MLP Heads ###")
print(f"  Tested on FavoriteColor (SAME): {results_df[(results_df['heads_from']=='FavoriteColor RWKV') & (results_df['tested_on']=='FavoriteColor')]['switch_rate'].values[0]:.1f}% switch")
print(f"  Tested on LivesInCity (CROSS):  {results_df[(results_df['heads_from']=='FavoriteColor RWKV') & (results_df['tested_on']=='LivesInCity')]['switch_rate'].values[0]:.1f}% switch")

print("\n### LivesInCity RWKV MLP Heads ###")
print(f"  Tested on LivesInCity (SAME):   {results_df[(results_df['heads_from']=='LivesInCity RWKV') & (results_df['tested_on']=='LivesInCity')]['switch_rate'].values[0]:.1f}% switch")
print(f"  Tested on FavoriteColor (CROSS): {results_df[(results_df['heads_from']=='LivesInCity RWKV') & (results_df['tested_on']=='FavoriteColor')]['switch_rate'].values[0]:.1f}% switch")

# %%
results_df.to_csv('results/cross_dataset_comparison_rwkv_mlp.csv', index=False)
print(f"\nResults saved to results/cross_dataset_comparison_rwkv_mlp.csv")
