...
class RWKVStateExtractor:
    ...
    def patch_state(
        self,
        source_state: List[torch.Tensor],
        target_state: List[torch.Tensor],
        patch_spec: Dict[int, Optional[List[int]]],
    ) -> List[torch.Tensor]:
        """
        Create a new state with specific heads/layers patched from source into target.

        Args:
            source_state: State to copy FROM
            target_state: State to copy INTO (base state)
            patch_spec: Dict mapping layer_idx -> list of head indices (or None for all heads)
                       Example: {5: [0, 2], 6: None} patches heads 0,2 from layer 5, all heads from layer 6

        Returns:
            New state list with specified patches applied
        """
        result = [s.clone() for s in target_state]

        for layer_idx, heads in patch_spec.items():
            state_idx = layer_idx * 3 + 1

            if heads is None:
                result[state_idx] = source_state[state_idx].clone()
            else:
                for head_idx in heads:
                    result[state_idx][head_idx] = source_state[state_idx][head_idx].clone()
