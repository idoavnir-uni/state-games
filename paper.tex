...
% Title and Authors
% =============================================================================
\title{Probing SSM States: The Query Bottleneck in Sub-quadratic Models}
...
Before interpreting probe results, we verify that the probe architecture is meaningful. In \cref{sec:probe_not_strong}, we show that the probe is not overly powerful (it fails on most heads). In \cref{sec:natural_vs_nonnatural}, we demonstrate that the query direction matters, confirming the probe leverages the model's native memory mechanism.
...
We compare the \textit{natural probe} (querying from the right) against the \textit{non-natural probe} (querying from the left). \cref{fig:natural_vs_non} visualizes this comparison across the top 10 heads for each probe type. The results show a substantial gap: the best \textit{natural probe} reaches 96.3\%, while the best \textit{non-natural probe} achieves only 70.8\%.

This confirms that memory is encoded in a manner consistent with the model's native query mechanism (\cref{eq:state_query}). Probing from the right extracts information effectively; probing from the left does not.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{natural_vs_non.png}
\caption{Comparison of \textit{natural probe} (querying from the right) versus \textit{non-natural probe} (querying from the left) accuracy on the Favorite Color dataset. Each point represents the best probe for a given head, ranked by accuracy. The natural probe consistently outperforms the non-natural probe, with the gap widening at higher ranks.}
...
\centering
\small
\caption{Counterfactual patching results (50 pairs, averaged over 5 seeds). ``Correct'': original property; ``Switched'': counterfactual property; ``Neither'': other output.}
\label{tab:causal}
\begin{tabular}{@{}lccc@{}}
\toprule
...
We find substantial overlap: \textbf{7 of the top 10 heads are shared across both datasets}. \cref{fig:universal_heads} visualizes this overlap, showing that the majority of top-performing heads are consistent across semantically distinct tasks. This suggests the existence of \emph{universal memory heads} that specialize in entity-property binding regardless of semantic content. We defer investigation of the mechanism underlying these heads to future work.
...
We investigate how context affects memory retention by measuring probe accuracy as a function of token distance from the information. \TODO{add figures} We observe that \textbf{increased semantic similarity between the prompt content and the target subject accelerates the model's forgetting rate}. When subsequent sentences discuss similar entities, probe accuracy degrades faster than when the intervening content is semantically distinct.
...
\textbf{Broader model coverage.} This work focuses exclusively on RWKV-7 (1.5B parameters). Testing whether our findings generalize to other SSM-based architectures (e.g., Mamba-2, GLA, DeltaNet) and different model scales would strengthen the conclusions and reveal whether the query bottleneck is a fundamental limitation of the SSM paradigm or specific to particular implementations.

\textbf{More robust evaluation.} Our datasets are limited to entity-property binding with relatively simple structure. Extending to more diverse semantic relationships, longer contexts, and naturalistic text would test whether the memory mechanism we identified operates in realistic scenarios. Additionally, developing more sophisticated probing methods could provide finer-grained insights into what information is stored and how.

\textbf{Understanding universal memory heads.} We identified heads that perform well across multiple semantic domains, but the mechanism underlying their universality remains unclear. Characterizing what makes these heads special could inform targeted architectural improvements or training objectives that enhance in-context memory.

\textbf{Improving the query mechanism.} Given that the bottleneck appears to be querying rather than storage, designing better query mechanisms is a natural next step. This could involve architectural modifications, training interventions, or post-hoc methods to improve how models retrieve information from their internal states.

\textbf{Semantic collision dynamics.} While we observed that semantic similarity accelerates forgetting, a deeper investigation into the mechanisms and potential mitigations would be valuable for understanding the limits of in-context memory.
